...
.DESCRIPTION
    ...
    2. Configures .wslconfig for mirrored networking (Chrome support).
    3. Bootstraps the WSL instance with all dependencies (CUDA, llama.cpp, Fara).
    4. Downloads models (User Interactive Selection) and sets up Python venvs.
#>
...
Write-Host "`n--- Step 2: Configuring .wslconfig ---" -ForegroundColor Cyan
$wslConfigContent = @"
[wsl2]
...
if (Test-Path $WSLConfigPath) {
    ...
    if ($currentConfig -match "networkingMode=mirrored") {
        Write-Host ".wslconfig already configured." -ForegroundColor Green
    } else {
        Write-Host "Updating .wslconfig (backing up old one)..."
        Copy-Item $WSLConfigPath "$WSLConfigPath.bak"
        Set-Content -Path $WSLConfigPath -Value $wslConfigContent
        Write-Host "Configuration updated. Restarting WSL..."
        wsl --shutdown
    }
} else {
...
Write-Host "`n--- Step 3: Select Model Quantization ---" -ForegroundColor Cyan
...
# --- Step 4: Define the Linux Provisioning Script ---
...
DEPS="build-essential cmake curl git libcurl4-openssl-dev nvidia-cuda-toolkit python3 python3-pip python3.12-venv direnv"
...
# --- Step 5: Execute Linux Script inside WSL ---
Write-Host "`n--- Step 5: Running Provisioning inside $DistroName ---" -ForegroundColor Cyan
